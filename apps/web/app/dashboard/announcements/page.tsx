...
export default function AnnouncementsPage() {
  ...
  async function handleSubmit(
    ...
    try {
      ...
      const publishAt = form.publishAt
      ...
      if (
        publishAt &&
        expiresAt &&
        expiresAt.getTime() <= publishAt.getTime()
      ) {
  ...
  async function handleArchive(
    ...
    try {
      ...
      setSuccess(
        "Announcement archived. It is stored under the Archived filter (not deleted).",
      );
    ...
    <div className="space-y-8">
      ...
      <section className="grid gap-5 sm:grid-cols-2 xl:grid-cols-5">
        <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-5 text-sm text-slate-500">Total Announcements</p>
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-5 text-sm text-slate-500">Published</p>
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-5 text-sm text-slate-500">Drafts</p>
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-5 text-sm text-slate-500">Archived</p>
          <p className="mt-2 text-3xl font-bold text-slate-900">{archivedCount}</p>
        </article>
        <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="w-fit rounded-xl bg-blue-50 p-3 text-blue-700">
            <Pin size={22} />
          </div>
          <p className="mt-5 text-sm text-slate-500">Pinned</p>
          <p className="mt-2 text-3xl font-bold text-slate-900">{pinnedCount}</p>
        </article>
      </section>
      ...
      {canManage ? (
        <div className="flex flex-wrap gap-2">
          {(
            ...
            >
              {option.label}
              {option.id === "ARCHIVED" ? ` (${archivedCount})` : ""}
            </button>
          ))}
        </div>
      ) : null}
      ...
      {visibleAnnouncements.length === 0 ? (
        <section className="rounded-2xl border border-slate-200 bg-white px-6 py-16 text-center shadow-sm">
          <Megaphone size={46} className="mx-auto text-slate-300" />
          ...
          <p className="mt-1 text-sm text-slate-500">
            {statusFilter === "ARCHIVED"
              ? "When you archive a published announcement, it appears here. It is not deleted."
          ...
          {visibleAnnouncements.map((announcement) => (
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-2 pr-12">
                <span className={statusTone(announcement.status)}>
                  {announcement.status}
                </span>
              ...
              <div className="mt-6 grid gap-3 rounded-xl bg-slate-50 p-4 text-sm text-slate-600 sm:grid-cols-2">
                <div className="flex gap-2">
                  <CalendarDays size={17} className="mt-0.5 shrink-0 text-slate-400" />
                  <div>
                    <p className="font-medium text-slate-700">Publish date</p>
                    <p className="mt-1 text-xs">{formatDate(announcement.publishAt)}</p>
                  </div>
                </div>
                <div className="flex gap-2">
                  <CalendarDays size={17} className="mt-0.5 shrink-0 text-slate-400" />
                  <div>
                    <p className="font-medium text-slate-700">Expiry date</p>
                    <p className="mt-1 text-xs">{formatDate(announcement.expiresAt)}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-5 border-t border-slate-100 pt-4">
                <p className="text-xs text-slate-500">
                  ...
                  <span className="font-medium text-slate-700">
                    {announcement.createdBy.employee
          ...
          <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
            <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-200 bg-white px-6 py-5">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Add company news, notices, or important updates.
                </p>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="space-y-6 p-6">
              ...
              <div className="grid gap-5 sm:grid-cols-2 sm:items-stretch">
                ...
                <div className="flex h-full flex-col">
                  ...
                  <label className="flex min-h-[4.25rem] flex-1 cursor-pointer items-center gap-3 rounded-xl border border-slate-300 px-4 py-3">
                    <input
                      type="checkbox"
              ...
              <div className="space-y-4 rounded-xl border border-slate-200 bg-slate-50/70 p-4">
                ...
                <DateTimeField
                  ...
                  onChange={(value) => updateForm("publishAt", value)}
                  onClear={() => updateForm("publishAt", "")}
                />
                <DateTimeField
                  label="Expiry date & time"
                  value={form.expiresAt}
                  minDate={splitDateTime(form.publishAt).date || undefined}
                  onChange={(value) => updateForm("expiresAt", value)}
                  onClear={() => updateForm("expiresAt", "")}
                />
              </div>
